...
\textit{API Key} at  for the access to \textit{MapTiler} and insert it as \texttt{api\_key}.
...
intersection in the image and return its coordinates as lattitude and longitude.

These values can then be used to download the nearest streetview panorama from
the given panoramax instance. Currently there is no \textit{API Key} needed to
...
the streetview panroama and the crops of the streetview panorama are written
to the file system.
...
problems with the MapTiler API, as we could stay below the free limit.
...
download of an adjacent street view panorama from Panoramax also works faultless.
...
fine tuning.
During our tests, we only achieved a precision of 65.79 percent and a recall of
around 53 percent.
The F1-Score of our solution equals to 57.45 percent.
...
relevant sections of the street view panorama. Our test show, that the current
...
of their respective models on Hugging-Face.
